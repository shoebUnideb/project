...
from accounts.models import CustomUser, SSOCode
from accounts.serializers import UserSerializer, UserAdminSerializer, CustomTokenObtainPairSerializer


def _rate_limited(request, group, rate='10/m'):
    return is_ratelimited(request._request, group=group, key='ip', rate=rate, increment=True)
...
def api_register(request):
    if _rate_limited(request, group='register', rate='5/m'):
...
def api_login(request):
    if _rate_limited(request, group='login', rate='10/m'):
        return Response({'detail': 'Too many requests. Try again later.'}, status=429)
...
def api_sso_exchange(request):
    ...
    """
    if _rate_limited(request, group='sso_exchange', rate='20/m'):
        return Response({'detail': 'Too many requests. Try again later.'}, status=429)
