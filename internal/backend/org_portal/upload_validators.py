...
def validate_lesson_content(file):
    if file.size > 200 * 1024 * 1024:
        raise serializers.ValidationError('Lesson content must be under 200 MB.')
    mime = _detect_mime(file)
    if mime not in (_DOC_TYPES | {'video/mp4'}):
        raise serializers.ValidationError(
            'Unsupported lesson content type. Allowed: PDF, DOCX, XLSX, PPTX, MP4.'
        )
    return file
