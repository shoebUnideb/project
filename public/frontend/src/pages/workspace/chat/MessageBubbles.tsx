...
import {
  Reply, Pin, MoreHorizontal, Star, Trash, Edit2, Forward, Copy, Check,
} from 'lucide-react';
import Avatar from '../../../components/ui/Avatar';
import { relativeTime } from '../../../utils/time';
import { sanitizeHtml } from '../../../utils/sanitize';
import type { WorkspaceChatMessage, WorkspaceDMMessage } from '../../../types';
import { FileAttachmentCard, ReactionRow, EmojiPickerPopup } from './ChatPrimitives';
import { MessageContextMenu, type ContextMenuAction } from './MessageContextMenu';
import { senderName } from './chatUtils';

export function MessageBubble({
  msg, currentUserId, isOwner, isStarred, onReact, onPin, onDelete, onReply, onEdit, onForward, onStar,
}: {
  ...
  const contextActions: ContextMenuAction[] = [
    ...
    ...(isMe ? [
      ...
      { label: 'Unsend', icon: <Trash size={13} />, onClick: onDelete, danger: true },
    ] : []),
    ...(!isMe && isOwner ? [
      { label: 'Delete', icon: <Trash size={13} />, onClick: onDelete, danger: true },
    ] : []),
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <ReactionRow reactions={msg.reactions} currentUserId={currentUserId} onReact={onReact} />
      </div>
      ...
      {ctxMenu && (
        <MessageContextMenu x={ctxMenu.x} y={ctxMenu.y} actions={contextActions} onClose={() => setCtxMenu(null)} />
      )}
    </div>
  );
}
...
export function DMMessageBubble({
  ...
  const contextActions: ContextMenuAction[] = [
    ...
    ...(isMe ? [
      { label: 'Edit',   icon: <Edit2 size={13} />, onClick: () => setEditing(true) },
      { label: 'Unsend', icon: <Trash size={13} />, onClick: onUnsend, danger: true },
    ] : []),
  ];
