...
import type { WorkspaceChatChannel } from '../../../types';

export function CreateChannelModal({ workspaceId, onClose, onCreated }: {
  workspaceId: number;
  onClose: () => void;
  onCreated: (ch: WorkspaceChatChannel) => void;
}) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const ch = await workspacesApi.createChannel(workspaceId, { name: name.trim(), description: desc });
      onCreated(ch);
...
export function EditChannelModal({ channel, workspaceId, onClose, onSaved }: {
  channel: WorkspaceChatChannel;
  workspaceId: number;
  onClose: () => void;
  onSaved: (ch: WorkspaceChatChannel) => void;
}) {
  ...
  const submit = async () => {
    ...
    try {
      await apiClient.initCsrf();
      const updated = await workspacesApi.updateChannel(workspaceId, channel.id, {
        name: name.trim(),
        description: desc,
      });
      onSaved(updated);
