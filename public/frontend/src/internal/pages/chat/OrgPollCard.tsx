...
import type { OrgChatPoll } from '../../api/orgApi';
import { senderName } from './OrgChatUtils';

export function OrgPollCard({
  poll, channelId, currentUserId, isAdmin, onVote, onClose, onDelete,
}: {
  poll: OrgChatPoll;
  channelId: number;
  ...
  const authorName = senderName(poll.author);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-3 space-y-2">
      <div className="flex items-start justify-between gap-2">
        ...
        {(isAdmin || isAuthor) && !poll.is_closed && (
      ...
      <div className="space-y-1.5">
        {poll.options.map(opt => {
          const myVote = opt.voter_ids.includes(currentUserId);
          const pct = poll.total_votes > 0 ? Math.round((opt.voter_count / poll.total_votes) * 100) : 0;
          return (
            <button
              key={opt.id}
              ...
              className={`w-full text-left relative px-3 py-2 rounded-lg border transition-colors ${
                myVote ? 'border-primary-400 bg-primary-50' : 'border-gray-200 bg-gray-50 hover:bg-gray-100'
            ...
            >
              ...
              <div className="relative flex items-center justify-between">
                ...
                <span className="flex items-center gap-1 text-[11px] text-gray-500">
                  {myVote && <Check size={11} className="text-primary-600" />}
                  {opt.voter_count} · {pct}%
                </span>
              </div>
            </button>
...
export function OrgCreatePollModal({ channelId, onClose, onCreated }: {
  channelId: number;
  onClose: () => void;
  onCreated: (poll: OrgChatPoll) => void;
}) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const poll = await orgChatApi.createPoll(channelId, {
