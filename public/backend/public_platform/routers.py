"""
Database router for public-portal.

Routes accounts and token_blacklist reads/writes to auth_db (shared with auth-service).
Everything else (core models) goes to the default database (gile_public).

Migration strategy: accounts tables are created on BOTH default and auth_db so that
PostgreSQL FK constraints from core_* tables to accounts_customuser resolve locally.
At runtime, all account reads/writes are routed to auth_db — the shadow tables on
gile_public remain empty and are never read. token_blacklist migrates on auth_db only.
"""
